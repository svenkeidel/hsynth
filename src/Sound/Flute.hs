...
flute dur amp freq pressure breath g rate =
  let env1   = lineSeg [(0,0.06),(1.1 * pressure,0.2),(pressure,dur - 0.16),(pressure,0.02)] 0 rate
      env2   = lineSeg [(0,0.01),(1,dur-0.02),(pressure,0.01)] 0 rate
      envib  = lineSeg [(0,0.5),(0,0.5),(1,dur - 1)] 1 rate
      flow   = random g * env1
      vibr   = sinA 5 rate * vibrato * envib
      bore   = delay (1 / freq) rate
      emb    = delay (1 / freq / 2) rate
      flute' = bore out
